...
class LianhanghaoSpider(scrapy.Spider):
    ...
    city_now = "35"
    city_name_now = u"北京市"
    bank_list = [('2', '中国农业银行'), ('3', '中国银行'), ('4', '中国建设银行'), ('5', '国家开发银行'),
    ...
    def parse(self, response):
        ...
        if self.first:
            for bank_tuple in self.bank_list:
                self.first = False
                for province, province_name in self.province_dict.iteritems():
                    ...
                    for item in city_list:
                        ...
                        bank_name_now = bank_tuple[1]
                        provice_now = province
                        province_name_now = province_name
                        city_now = city
                        city_name_now = item['name']
                        new_start_url = '%s/index.php?&key=&bank=%s&province=%s&city=%s&page=1' % (
                            self.domain, bank_now, provice_now, city_now)
                        # 取得当前分类 pages
    ...
    def get_param(self, response):
        ...
        # http://www.lianhanghao.com/index.php?&key=&bank=1&province=1&city=35&page=1
        url = response.url
        ...
        city_list = self.get_city(province_code)
        for item in city_list:
            if item['id'] == city:
                city_name = item['name']
                break
